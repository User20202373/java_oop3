...
public class HttpPutParsingEx {
    ...
    public static void main(String[] args) {
        ...
        // 사전기반지식: 통신을 요청하는 코드에서는 거의 대부분 예외처리를 해 주어야 한다
        try {
            Post updatePost = Post.builder()
                    ...
                    .userId(10)
                    .build();
            String jsonRequest = gson.toJson(updatePost); // 직렬화

            // Http 요청 메시지 생성
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create("https://jsonplaceholder.typicode.com/posts/1"))
                    .header("Content-Type", "application/json")
                    //서버측으로 보낼 타입을 명시하는 곳(HTTP body 영역)
                    .PUT(HttpRequest.BodyPublishers.ofString(jsonRequest))  // 수정이니까 PUt
                    .build();

            // 서버측으로 요청 전송 및 응답
            //1 번째 인수값: 내가 만들어 둔 HTTP 요청 메세지 객체
            // 2번째 인수 값: 서버가 보내준 데이터를 어떤 형태로 받을 것인가 명시(String)
